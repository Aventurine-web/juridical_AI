...
/* Abstract strategy orb — flowing, connected, alive. Same shape and motion the
   team liked, recolored to warm apricot / coral / rose / burnt-orange and tuned
   for a premium, tactile feel (low iridescence, no neon). Reacts subtly to the
   cursor; click to open the AI strategist. */
...
// Warm studio lighting: cream, apricot, coral, rose, burnt orange, deep umber.
const PANELS: Panel[] = [
  { color: 0xfff3e9, intensity: 4.6, size: [16, 5], position: [0, 8, 3] },     // warm key
  { color: 0xfff0e2, intensity: 2.4, size: [4, 10], position: [8, 2, 6] },     // warm fill
  { color: 0xf5b088, intensity: 2.6, size: [10, 8], position: [-9, 1, 3] },    // apricot
  { color: 0xe97a62, intensity: 2.2, size: [8, 9], position: [9, 2, -5] },     // coral
  { color: 0xd98a9c, intensity: 1.9, size: [10, 3], position: [-4, -7, 6] },   // rose
  { color: 0xd36a3d, intensity: 1.7, size: [3, 6], position: [3, 6, -8] },     // burnt orange
  { color: 0x7a3b2a, intensity: 1.0, size: [12, 12], position: [0, -9, -2] },  // deep umber
]
...
function OrbMesh({ hovered, pointer }: { hovered: boolean; pointer: React.RefObject<Pointer> }) {
  ...
  const { shellMaterial, coreMaterial, envMap } = useMemo(() => {
    ...
    // Warm frosted-glass shell. Iridescence kept low so it stays warm (no blue sheen).
    ...
    // Warm coral/rose core gives the orb its inner glow.
  ...
  useFrame((_, delta) => {
    ...
    speed.current = THREE.MathUtils.damp(speed.current, hovered ? 1.2 : 1.0, 3, delta)
    shellU.uTime.value += delta * speed.current
    coreU.uTime.value += delta * speed.current * 1.3
    shellU.uDistort.value = THREE.MathUtils.damp(shellU.uDistort.value, hovered ? 0.155 : 0.13, 3, delta)

    // Subtle cursor-follow tilt.
    const p = pointer.current ?? { x: 0, y: 0 }
    tilt.current.x = THREE.MathUtils.damp(tilt.current.x, p.y * 0.22, 4, delta)
    tilt.current.y = THREE.MathUtils.damp(tilt.current.y, p.x * 0.3, 4, delta)
    ...
    if (group.current) {
      ...
      group.current.rotation.y = t * 0.06 + tilt.current.y
      group.current.rotation.x = Math.sin(t * 0.11) * 0.08 + tilt.current.x
      group.current.position.y = Math.sin(t * 0.35) * 0.04
      const breathe = 1 + Math.sin(t * 0.45) * 0.008
      const s = THREE.MathUtils.damp(group.current.scale.x / breathe, hovered ? 1.025 : 1.0, 4, delta)
...
export function IridescentOrb({ onActivate }: { onActivate: () => void }) {
  ...
  return (
    <div
      className={`relative h-full w-full ${hovered ? "cursor-pointer" : ""}`}
    ...
    >
      ...
      >
        <group
          ...
          onClick={onActivate}
        >
          <OrbMesh hovered={hovered} pointer={pointer} />
        </group>
      </Canvas>
      <button
        onClick={onActivate}
        className="absolute inset-x-0 bottom-2 mx-auto flex w-fit items-center gap-2 rounded-full border border-border bg-popover/80 px-4 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur-sm transition hover:text-foreground"
      >
        <span className="h-1.5 w-1.5 rounded-full bg-primary" />
        {hovered ? "Tap to meet your strategist" : "Interactive · ask it anything"}
      </button>
    </div>
  )
}
